# Thumbnails de Referência

Miniaturas coletadas do canal **@HistoriasdaBiblia.oficial** utilizadas para extração do DNA visual (VDNA).

Essas imagens servem como base para identificar:

- padrões de composição
- arquétipos emocionais
- iluminação cinematográfica
- escala de personagem

---

## Exemplo de Thumbnail Viral

![Thumbnail analisada](thumb1.png)

---

### Características observadas

- personagem dominante na composição
- iluminação dramática cinematográfica
- cenário épico ou bíblico
- forte contraste entre personagem e fundo
- narrativa visual clara
